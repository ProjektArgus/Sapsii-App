...
# Supabase's own superuser is supabase_admin; the postgres role in that image is
# not a superuser, so it cannot create a database owned by another role.
# $SAPSII_DB_PASSWORD is expanded by the container's shell, not this one.
# shellcheck disable=SC2016
docker exec -i \
  -e SAPSII_DB_PASSWORD="$SAPSII_DB_PASSWORD" \
  supabase-db sh -eu -c 'psql -U supabase_admin -d postgres -v ON_ERROR_STOP=1 --set=db_password="$SAPSII_DB_PASSWORD"' <<'SQL'
